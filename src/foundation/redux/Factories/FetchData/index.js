...
class FetchData extends React.Component {
  ...
  resetForNewRequestIfNeeded() {
    const { stateSubID, sendData } = this.props;

    // check that there is some sent data present on the state
    if (dNc(this.props.reduxState_this[stateSubID]) && dNc(this.props.reduxState_this[stateSubID].sentData)) {
  ...
  render() {
    ...
    if (dNc(this.props.reduxState_this[stateSubID])) {
      ({ generalStatus, payload } = this.props.reduxState_this[stateSubID]);
    }
    ...
    if (generalStatus === 'fatal') {
      ...
      if (typeof Viewer === 'object') {
        ...
        component = <Viewer data={payload} {...this.props.viewerProps} />;
      }
    } else {
...
FetchData.propTypes = {
  ...
  refreshComparator: PropTypes.object,
